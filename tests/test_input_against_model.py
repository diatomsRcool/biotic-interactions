...
from biotic_interactions.interaction import Interaction
            ...
            for fname in files:
                ...
                try:
                    if fname.endswith('.yaml'):
                        nyaml += 1
                        o: Interaction = yaml_loader.load(full_fname, Interaction)
                        pyaml += 1
                    elif fname.endswith('.json'):
                        njson += 1
                        o: Interaction = json_loader.load(full_fname, Interaction)
                        pjson += 1
                    elif fname.endswith('.ttl'):
                        nttl += 1
                        o: Interaction = rdf_loader.load(full_fname, Interaction)
                        pttl += 1
